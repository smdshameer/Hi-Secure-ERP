...
import { IconChevronLeft, IconPlus, IconTrash, IconCalculator, IconFileDescription, IconTruck, IconUsers, IconSquarePlus, IconAlertCircle, IconCalendar, IconDeviceFloppy, IconFileCheck } from '@tabler/icons-react';
...
export default function DeliveryChallanForm({ backPath }: { backPath: string }) {
  ...
  // Dispatch Destination Type: 'customer', 'supplier', or 'branch'
  const [dispatchType, setDispatchType] = useState<'customer' | 'supplier' | 'branch'>('customer');
  ...
  // Selected Entity details for preview
  ...
  const selectedToLocation = locations.find(l => l.location_id === form.to_location_id);

  useEffect(() => {
    setLoading(true);
    const fetchDeps = async () => {
      try {
        const [custRes, suppRes, locRes, partRes, brandRes] = await Promise.all([
          api.get('/customers'),
          api.get('/suppliers'),
          api.get('/locations'),
          api.get('/parts'),
        ...
        if (isEdit) {
          const res = await api.get(`/delivery-challans/${id}`);
          const dc = res.data;
          if (dc) {
            setForm({
              customer_id: dc.customer_id ?? 0,
              supplier_id: dc.supplier_id ?? 0,
              from_location_id: dc.from_location_id ?? 0,
              to_location_id: dc.to_location_id ?? 0,
              challan_date: dc.challan_date ? new Date(dc.challan_date).toISOString().split('T')[0] : '',
              expected_delivery_date: dc.expected_delivery_date ? new Date(dc.expected_delivery_date).toISOString().split('T')[0] : '',
              ...
              purposes: dc.purposes || 'Branch Transfer',
              status: dc.status || 'draft',
              notes: dc.notes || '',
            });

            if (dc.customer_id) setDispatchType('customer');
            else if (dc.supplier_id) setDispatchType('supplier');
            else if (dc.to_location_id) setDispatchType('branch');

            if (dc.items) {
              setLines(dc.items.map((i: any) => ({
                part_id: i.part_id,
  ...
  const handleQuickCustomerSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, customer_id: newCustId }));
  ...
  const totalValuation = lines.reduce((sum, item) => sum + (Number(item.quantity || 0) * Number(item.unit_price || 0)), 0);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const filteredItems = lines.filter(l => l.part_id > 0 && l.quantity > 0);
    ...
    const postData = {
      ...form,
      customer_id: dispatchType === 'customer' ? Number(form.customer_id) : null,
      supplier_id: dispatchType === 'supplier' ? Number(form.supplier_id) : null,
      from_location_id: Number(form.from_location_id) || null,
      to_location_id: dispatchType === 'branch' ? Number(form.to_location_id) : null,
    ...
    try {
      if (isEdit) {
        await api.put(`/delivery-challans/${id}`, postData);
      } else {
        await api.post('/delivery-challans', postData);
      }
      ...
      >
        <div>
          <h1 className="text-2xl font-extrabold flex items-center gap-2.5">
            <IconTruck size={26} />
            {isEdit ? 'Modify Delivery Challan' : 'New Delivery Challan'}
          </h1>
          <p className="text-[13px] opacity-90 mt-1">
            {isEdit ? 'Configure and update material dispatch slip' : 'Create and issue a new delivery challan'}
          </p>
        </div>
        <Link
          to={backPath}
        ...
        >
          <IconChevronLeft size={16} /> Back to Challans
        </Link>
      </div>

      {loading && <div className="text-center py-20 text-gray-400">Loading challan details...</div>}
        ...
        <form onSubmit={handleSubmit} className="lg:flex-1 lg:min-h-0 lg:flex lg:flex-col">
          <div className="grid grid-cols-1 lg:grid-cols-[78fr_22fr] gap-6 lg:flex-1 lg:min-h-0">
            ...
            <div className="space-y-6 pb-6 lg:h-full lg:overflow-y-auto lg:overscroll-contain no-scrollbar pr-1">
              ...
              {/* LOGISTICS AND TARGET DESTINATION */}
              <div className="grid grid-cols-1 md:grid-cols-[60fr_40fr] gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:border-blue-200">
                  <div className="bg-gradient-to-r from-violet-50 to-purple-50 border-b border-gray-200 px-4 py-2.5 flex items-center gap-2">
                    <div className="w-6 h-6 bg-violet-600 rounded-md flex items-center justify-center">
                      <IconTruck size={13} className="text-white" />
                    </div>
                    <span className="font-bold text-gray-800 text-[12.5px]">Logistics &amp; Transport Details</span>
                  </div>

                  <div className="p-4 space-y-3.5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Challan Date <span className="text-red-500">*</span></label>
                      ...
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Expected Delivery</label>
                        <div className="relative flex items-center">
                          <input
                            type="date"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Vehicle Number</label>
                        <input
                          type="text"
                          value={form.vehicle_number}
                          onChange={e => setForm(p => ({ ...p, vehicle_number: e.target.value }))}
                          placeholder="e.g. DL 1CA 1234"
                          className="w-full border border-gray-200 rounded-lg px-3 h-[36px] text-[11.5px] text-gray-850 outline-none transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)] bg-gray-50 focus:bg-white font-mono tracking-wide"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Transporter</label>
                        <input
                          type="text"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Driver Name</label>
                        <input
                          type="text"
                      ...
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">E-Way Bill Number</label>
                        <input
                          type="text"
                          value={form.eway_bill_number}
                          onChange={e => setForm(p => ({ ...p, eway_bill_number: e.target.value }))}
                          placeholder="12-digit EWB No"
                          className="w-full border border-gray-200 rounded-lg px-3 h-[36px] text-[11.5px] text-gray-850 outline-none transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)] bg-gray-50 focus:bg-white font-mono"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Purpose <span className="text-red-500">*</span></label>
                        <select
                          value={form.purposes}
                          onChange={e => setForm(p => ({ ...p, purposes: e.target.value }))}
                        ...
                        >
                          ...
                          <option value="Job Work">Job Work (Repair/Process)</option>
                          <option value="Demo / Exhibition">Demo / Exhibition</option>
                          <option value="Sales Return">Sales Return</option>
                          <option value="Dispatch for Sale">Dispatch for Sale</option>
                        </select>
                      </div>

                      <div className="space-y-1">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">From Location Branch <span className="text-red-500">*</span></label>
                        <select
                          value={form.from_location_id}
                          onChange={e => setForm(p => ({ ...p, from_location_id: Number(e.target.value) }))}
                          required
                          className="w-full border border-gray-200 rounded-lg px-3.5 h-[36px] text-[11.5px] text-gray-700 outline-none transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)] bg-gray-50 focus:bg-white"
                        >
                ...
                {/* DESTINATION PROFILE */}
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:border-blue-200">
                  <div className="bg-gradient-to-r from-violet-50 to-purple-50 border-b border-gray-200 px-4 py-2.5 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 bg-violet-600 rounded-md flex items-center justify-center">
                        <IconUsers size={13} className="text-white" />
                      </div>
                      <span className="font-bold text-gray-800 text-[12.5px]">Destination Profile</span>
                    </div>
                    ...
                    {dispatchType === 'customer' && (
                      <button 
                        ...
                        className="text-[10px] font-bold text-blue-650 hover:underline flex items-center gap-0.5"
                      >
                        <IconSquarePlus size={13} /> Add Cust
                      </button>
                    )}
                    {dispatchType === 'supplier' && (
                      <button 
                        type="button" 
                        onClick={() => setShowSuppModal(true)}
                        className="text-[10px] font-bold text-blue-650 hover:underline flex items-center gap-0.5"
                      >
                        <IconSquarePlus size={13} /> Add Supp
                      </button>
                    )}
                  </div>
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {/* Destination Toggle */}
                    <div className="grid grid-cols-3 gap-1 p-1 bg-gray-550 border border-gray-200 rounded-lg text-center text-[10px] font-bold text-gray-600">
                      <button 
                        type="button" 
                        onClick={() => setDispatchType('customer')}
                        className={`py-1.5 rounded-md transition-colors ${dispatchType === 'customer' ? 'bg-[#1a3480] text-white shadow-sm' : 'hover:bg-gray-100'}`}
                      >
                        Customer
                      </button>
                      <button 
                        type="button" 
                        onClick={() => setDispatchType('supplier')}
                        className={`py-1.5 rounded-md transition-colors ${dispatchType === 'supplier' ? 'bg-[#1a3480] text-white shadow-sm' : 'hover:bg-gray-100'}`}
                      >
                        Supplier
                      </button>
                      <button 
                        type="button" 
                        onClick={() => setDispatchType('branch')}
                        className={`py-1.5 rounded-md transition-colors ${dispatchType === 'branch' ? 'bg-[#1a3480] text-white shadow-sm' : 'hover:bg-gray-100'}`}
                      >
                        Branch
                      </button>
                    </div>

                    {/* Customer Dispatch Selection */}
                    {dispatchType === 'customer' && (
                      <div className="space-y-3">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Select Customer *</label>
                          <select 
                            ...
                            onChange={e => {
                              if (e.target.value === '__new__') {
                                ...
                                setForm(p => ({ ...p, customer_id: Number(e.target.value) }));
                              }
                            }}
                            required
                            className="w-full border border-gray-200 rounded-lg px-3.5 h-[36px] text-[11.5px] text-gray-700 outline-none transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)] bg-gray-50 focus:bg-white"
                        ...
                        {selectedCustomer ? (
                          <div className="bg-gray-50 border border-gray-150 rounded-lg p-2.5 text-[11px] text-gray-600 space-y-1 leading-normal">
                            <div><span className="font-semibold text-gray-700">Recipient:</span> {selectedCustomer.name}</div>
                            {selectedCustomer.phone && <div><span className="font-semibold text-gray-700">Mobile:</span> {selectedCustomer.phone}</div>}
                            {selectedCustomer.gstin && <div><span className="font-semibold text-gray-700">GSTIN:</span> <span className="font-bold text-blue-750">{selectedCustomer.gstin}</span></div>}
                          ...
                          <div className="text-center py-4 text-gray-400 text-[11px] border border-dashed border-gray-200 rounded-lg">
                            Choose destination customer details
                          </div>
                        )}
                      </div>
                    )}

                    {/* Supplier Dispatch Selection */}
                    {dispatchType === 'supplier' && (
                      <div className="space-y-3">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Select Supplier *</label>
                          <select 
                        ...
                        {selectedSupplier ? (
                          <div className="bg-gray-50 border border-gray-150 rounded-lg p-2.5 text-[11px] text-gray-600 space-y-1 leading-normal">
                            <div><span className="font-semibold text-gray-700">Supplier:</span> {selectedSupplier.name}</div>
                            {selectedSupplier.phone && <div><span className="font-semibold text-gray-700">Mobile:</span> {selectedSupplier.phone}</div>}
                            {selectedSupplier.gstin && <div><span className="font-semibold text-gray-700">GSTIN:</span> <span className="font-bold text-blue-750">{selectedSupplier.gstin}</span></div>}
                          ...
                          <div className="text-center py-4 text-gray-400 text-[11px] border border-dashed border-gray-200 rounded-lg">
                            Choose destination supplier details
                          </div>
                        )}
                      </div>
                    )}

                    {/* Branch/Location Dispatch Selection */}
                    {dispatchType === 'branch' && (
                      <div className="space-y-3">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">To Location Branch *</label>
                          <select 
                            value={form.to_location_id} 
                            onChange={e => setForm(p => ({ ...p, to_location_id: Number(e.target.value) }))}
                            required
                            className="w-full border border-gray-200 rounded-lg px-3.5 h-[36px] text-[11.5px] text-gray-700 outline-none transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.12)] bg-gray-50 focus:bg-white"
                          >
                            <option value="0">Choose Location...</option>
                        ...
                        {selectedToLocation ? (
                          <div className="bg-gray-50 border border-gray-150 rounded-lg p-2.5 text-[11px] text-gray-600 space-y-1 leading-normal">
                            <div><span className="font-semibold text-gray-700">Branch:</span> {selectedToLocation.name}</div>
                            {selectedToLocation.address && <div><span className="font-semibold text-gray-700">Address:</span> {selectedToLocation.address}</div>}
                          </div>
                        ) : (
                          <div className="text-center py-4 text-gray-400 text-[11px] border border-dashed border-gray-200 rounded-lg">
                            Choose target branch destination
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <table className="w-full border-collapse text-[11.5px]" style={{ tableLayout: 'fixed' }}>
                    ...
                    <thead>
                      <tr className="bg-gray-50 text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">
                        ...
                        <th className="py-2.5 px-2 text-right">Declared Price</th>
                        <th className="py-2.5 px-2 text-left">Remarks</th>
                        ...
                        <tr>
                          <td colSpan={6} className="py-8 text-center text-gray-400">
                            <div className="flex flex-col items-center justify-center p-3 space-y-1">
                              ...
                              <p className="font-bold text-gray-600">No items dispatching yet</p>
                              <small className="text-gray-400">Click "+ Add Product" above to load items</small>
                            </div>
                          </td>
                        </tr>
                      ) : (
                        lines.map((line, idx) => {
                          ...
                          return (
                            <tr key={idx} className="hover:bg-blue-50/30 transition-colors duration-100">
                              ...
                              <td className="py-2.5 px-2 text-center font-medium text-gray-600">{line.batch_number || '—'}</td>
                              ...
                              <td className="py-2.5 px-2 text-left text-gray-600 truncate">{line.remarks || '—'}</td>
              ...
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:border-blue-200">
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-b border-gray-200 px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="font-bold text-gray-800 text-[13px]">Special Instructions / Declarations</span>
                </div>
                <div className="p-4 space-y-3.5">
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Declaration Remarks</label>
                    <textarea 
                      value={form.notes} 
                      onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} 
                      rows={4}
                      placeholder="Specify transport terms, job work parameters, non-commercial declaration statements..."
                  ...
                  {totalValuation > 0 && (
                    <div className="text-[10px] text-indigo-600 font-medium italic bg-indigo-50 px-2.5 py-1.5 rounded-lg border border-indigo-100/30">
                      {toRupeesInWords(totalValuation)}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4 transition-all duration-300 hover:shadow-md hover:border-blue-200">
                ...
                <div className="bg-gradient-to-b from-[#f8faff] to-[#f9fafb] border-2 border-gray-100 rounded-xl p-4 space-y-2.5">
                  <div className="flex justify-between text-[12px]">
                    ...
                    <span className="font-extrabold text-gray-800">₹{totalValuation.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {saving ? 'Processing...' : (isEdit ? 'Update Challan' : 'Create Challan')}
                  </button>
                ...
                <div className="tips-box bg-amber-50 border-l-4 border-amber-500 rounded-lg p-3.5 text-[11px] text-amber-900 leading-normal">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-amber-800/80">
                    <li>Declare valuation for E-Way bills correctly</li>
                    <li>Toggle Customer, Supplier, or Branch Destination</li>
                    <li>Delivery challans do not create accounting ledger balances</li>
                  </ul>
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[3000] p-4">
          <div className="bg-white rounded-xl shadow-xl border border-gray-250 max-w-md w-full p-6 space-y-4">
            <h3 className="text-[15px] font-bold text-gray-800 border-b border-gray-100 pb-2 flex items-center gap-1.5">
              <IconPlus className="text-blue-600" size={18} /> Add Challan Item
            </h3>
            <div className="space-y-3">
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-600 mb-1">Declared Price (₹) *</label>
                  <input 
                    type="number" 
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-600 mb-1">Batch Code / Number</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-600 mb-1">Remarks</label>
                <input 
                  type="text" 
                  value={newItem.remarks} 
                  onChange={e => setNewItem(prev => ({ ...prev, remarks: e.target.value }))}
                  placeholder="e.g. For client evaluation"
                  className="w-full border border-gray-305 rounded-lg px-3 h-[36px] text-[13px] outline-none transition-all duration-200 hover:border-gray-400 focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.15)] bg-white" 
                />
              </div>
