...
import {
  IconChevronLeft, IconUser, IconPhone, IconMail, IconMapPin,
  IconCertificate, IconCreditCard, IconHistory, IconFileText,
  IconFileDescription, IconTruck, IconTool, IconUsers, IconEdit,
...
export default function CustomerDetail() {
  ...
  const [activeTab, setActiveTab] = useState<'repairs' | 'invoices' | 'quotations' | 'challans'>('repairs');
  ...
  useEffect(() => {
    ...
    const cachedNotes = localStorage.getItem(`notes_customer_${id}`);
    if (cachedNotes) {
      try {
  ...
  if (loading) {
    return <div className="text-center py-20 text-gray-400">Loading customer details...</div>;
  }
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this customer? This will also delete all associated repair records.')) {
      try {
  ...
  const handleAddNote = () => {
    ...
    setNotes(updated);
    localStorage.setItem(`notes_customer_${id}`, JSON.stringify(updated));
    setNewNote('');
  };

  // Calculations for statistics
  const totalRepairs = customer.repairs?.length || 0;
  const lifetimeValue = customer.repairs?.reduce((sum, r) => sum + Number(r.actual_cost || 0), 0) || 0;
  const lastRepairDate = customer.repairs?.length > 0
    ? new Date(Math.max(...customer.repairs.map(r => new Date(r.received_date).getTime())))
    : null;
  ...
  const getRepairStatusBadgeClass = (status: string) => {
    switch ((status || '').toLowerCase()) {
      ...
      case 'diagnosed':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-50 text-amber-700 border border-amber-200';
      case 'awaiting_parts':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-pink-50 text-pink-700 border border-pink-200';
      case 'in_repair':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-indigo-50 text-indigo-700 border border-indigo-200';
      ...
      case 'completed':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-slate-100 text-slate-800 border border-slate-300';
      case 'cancelled':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-50 text-rose-700 border border-rose-250';
      default:
  ...
  const getInvoiceStatusBadgeClass = (status: string) => {
    switch ((status || '').toLowerCase()) {
      case 'paid':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-emerald-50 text-emerald-700 border border-emerald-200';
      case 'pending':
      case 'partial':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-50 text-amber-700 border border-amber-200';
      case 'draft':
      ...
      case 'cancelled':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-50 text-rose-700 border border-rose-250';
      default:
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-gray-50 text-gray-600 border border-gray-200';
    }
  };
  ...
  const getQuoteStatusBadgeClass = (status: string) => {
    switch ((status || '').toLowerCase()) {
      ...
      case 'rejected':
      case 'expired':
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-50 text-rose-700 border border-rose-200';
      default:
        return 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-gray-50 text-gray-600 border border-gray-200';
    }
  };
  ...
  return (
    <div className="max-w-[1600px] w-full mx-auto px-4 relative flex-1 min-h-0 flex flex-col gap-4 pb-6">
      ...
      <PageBanner
        ...
        subtitle={`Detailed view and records history for ${customer.name}`}
        ...
        action={
          <div className="flex gap-2 no-print">
            ...
            >
              <IconTrash size={15} /> Delete Customer
            </button>
          </div>
        }
      />
      ...
      <div className="flex-1 min-h-0 overflow-y-auto pr-1 grid-container">
        ...
        {/* UPPER GRID: Customer Info & Document Tabs */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2.2fr] gap-5 items-stretch print-block">
              ...
              <div className="p-5 space-y-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[9.5px] font-bold px-2 py-0.5 rounded uppercase bg-blue-50 text-blue-700 border border-blue-200">
                      {customer.customer_type}
                    </span>
                  </div>
                  <p className="text-[11px] text-gray-400 font-mono">Code: {customer.customer_code}</p>
                </div>
                  ...
                  <table className="w-full text-[13px] text-left text-gray-650 border-collapse">
                    <tbody>
                      ...
                      <tr className="border-b border-gray-50">
                        ...
                        <td className="py-2 font-mono text-gray-700 font-semibold">{customer.gstin || 'Unregistered'}</td>
                      </tr>
                      <tr className="border-b border-gray-50">
                        <th className="py-2 font-bold text-gray-500 flex items-center gap-1.5"><IconCreditCard size={14} /> Credit Limit</th>
                        <td className="py-2 font-bold text-red-600">
                          ₹{Number(customer.credit_limit || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td className="py-2">
                          {customer.is_active ? (
                            ...
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-50 text-rose-700 border border-rose-250">Inactive</span>
                          )}
                        </td>
                      </tr>
                    </tbody>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden card-wrapper flex flex-col h-full">
              ...
              <div className="px-5 py-3 border-b border-slate-200 bg-slate-50/80 flex items-center justify-start no-print">
                <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg w-full max-w-xl shadow-inner">
                  <button
                    type="button"
                  ...
                  >
                    <IconTool size={16} className="flex-shrink-0" />
                  ...
                  >
                    <IconFileDescription size={16} className="flex-shrink-0" />
                  ...
                  >
                    ...
                    <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-full ${
                      ...
                      {customer.deliveryChallans?.length || 0}
                    </span>
                  </button>
                </div>
              </div>
              ...
              <div className="p-0 min-h-[380px]">
                ...
                <div className={`${activeTab === 'repairs' ? 'block' : 'hidden'} tab-content-print-visible`}>
                  {/* Print-only Section Header */}
                  <div className="hidden print-section-header px-5 py-2">
                    Repair History ({customer.repairs?.length || 0})
                  </div>
                    ...
                    <table className="w-full text-[13px] text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-50/70 text-gray-500 font-bold uppercase tracking-wider text-[11px] border-b border-gray-100">
                          <th className="py-2.5 px-5">Ticket #</th>
                          <th className="py-2.5 px-5">Product Details</th>
                          <th className="py-2.5 px-5">Status</th>
                          <th className="py-2.5 px-5">Received</th>
                          <th className="py-2.5 px-5 text-right">Cost</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(!customer.repairs || customer.repairs.length === 0) ? (
                          ...
                          customer.repairs.map((rep) => (
                            <tr key={rep.repair_id} className="hover:bg-blue-50/30 transition-colors">
                              <td className="py-2.5 px-5 font-bold text-blue-700">{rep.ticket_number}</td>
                              <td className="py-2.5 px-5">
                                <span className="font-bold text-gray-855 block">{rep.product_type}</span>
                                {(rep.brand?.name || rep.model_number) && (
                                  <span className="text-[11.5px] text-gray-400 block mt-0.5">
                                    {rep.brand?.name || ''} {rep.model_number ? `· ${rep.model_number}` : ''}
                                  </span>
                                )}
                              </td>
                              <td className="py-2.5 px-5">
                                <span className={getRepairStatusBadgeClass(rep.repair_status)}>
                                  {rep.repair_status.replace('_', ' ')}
                                </span>
                              </td>
                              <td className="py-2.5 px-5 text-gray-500">
                                {new Date(rep.received_date).toLocaleDateString('en-IN')}
                              </td>
                              <td className="py-2.5 px-5 text-right font-bold text-gray-800">
                                ₹{Number(rep.actual_cost || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                              </td>
                              <td className="py-2.5 px-5 text-center no-print">
                                <Link
                                  to={`/repairs/${rep.repair_id}/edit`}
                                  className="inline-flex items-center justify-center p-1 rounded-md border border-gray-200 text-gray-500 hover:text-blue-600 hover:border-blue-300 transition-colors shadow-sm hover:shadow"
                                >
                                  <IconEdit size={13} />
                                </Link>
                              </td>
                ...
                <div className={`${activeTab === 'invoices' ? 'block' : 'hidden'} tab-content-print-visible`}>
                  {/* Print-only Section Header */}
                  <div className="hidden print-section-header px-5 py-2">
                    Recent Invoices ({customer.salesInvoices?.length || 0})
                  </div>
                  <div className="overflow-x-auto max-h-[380px] overflow-y-auto">
                    <table className="w-full text-[13px] text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-50/70 text-gray-500 font-bold uppercase tracking-wider text-[11px] border-b border-gray-100">
                          ...
                          <th className="py-2.5 px-5">Invoice Date</th>
                          <th className="py-2.5 px-5">Status</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(!customer.salesInvoices || customer.salesInvoices.length === 0) ? (
                          ...
                          customer.salesInvoices.map((inv) => (
                            <tr key={inv.invoice_id} className="hover:bg-blue-50/30 transition-colors">
                              <td className="py-2.5 px-5 font-bold text-blue-700">{inv.invoice_number}</td>
                              <td className="py-2.5 px-5 text-gray-500 font-medium">
                                {new Date(inv.invoice_date).toLocaleDateString('en-IN')}
                              </td>
                              <td className="py-2.5 px-5">
                                <span className={getInvoiceStatusBadgeClass(inv.status)}>
                                  {inv.status}
                                </span>
                              </td>
                              <td className="py-2.5 px-5 text-right font-extrabold text-gray-855">
                                ₹{Number(inv.grand_total || inv.total_amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                              </td>
                              <td className="py-2.5 px-5 text-center no-print">
                                <Link
                                  to={`/sales/${inv.invoice_id}`}
                ...
                <div className={`${activeTab === 'quotations' ? 'block' : 'hidden'} tab-content-print-visible`}>
                  {/* Print-only Section Header */}
                  <div className="hidden print-section-header px-5 py-2">
                    Recent Quotations ({customer.quotations?.length || 0})
                  </div>
                  <div className="overflow-x-auto max-h-[380px] overflow-y-auto">
                    <table className="w-full text-[13px] text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-55/65 text-gray-500 font-bold uppercase tracking-wider text-[11px] border-b border-gray-100">
                          <th className="py-2.5 px-5">Quote #</th>
                          <th className="py-2.5 px-5">Quote Date</th>
                          <th className="py-2.5 px-5">Status</th>
                          <th className="py-2.5 px-5 text-right">Amount</th>
                          <th className="py-2.5 px-5 text-center no-print">Action</th>
                        </tr>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(!customer.quotations || customer.quotations.length === 0) ? (
                          ...
                          customer.quotations.map((q) => (
                            <tr key={q.quote_id} className="hover:bg-blue-50/30 transition-colors">
                              <td className="py-2.5 px-5 font-bold text-blue-700">{q.quote_number}</td>
                              <td className="py-2.5 px-5 text-gray-500">
                                {new Date(q.quote_date || q.created_at).toLocaleDateString('en-IN')}
                              </td>
                              <td className="py-2.5 px-5">
                                <span className={getQuoteStatusBadgeClass(q.status)}>
                                  {q.status}
                                </span>
                              </td>
                              <td className="py-2.5 px-5 text-right font-extrabold text-gray-855">
                ...
                <div className={`${activeTab === 'challans' ? 'block' : 'hidden'} tab-content-print-visible`}>
                  {/* Print-only Section Header */}
                  <div className="hidden print-section-header px-5 py-2">
                    Delivery Challans ({customer.deliveryChallans?.length || 0})
                  </div>
                  <div className="overflow-x-auto max-h-[380px] overflow-y-auto">
                    <table className="w-full text-[13px] text-left border-collapse">
                      <thead>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(!customer.deliveryChallans || customer.deliveryChallans.length === 0) ? (
                          <tr>
                            <td colSpan={5} className="text-center py-8 text-gray-400 italic">
                          ...
                          customer.deliveryChallans.map((dc) => (
                            <tr key={dc.delivery_challan_id} className="hover:bg-blue-50/30 transition-colors">
                              <td className="py-2.5 px-5 font-bold text-blue-700">{dc.challan_number}</td>
                              <td className="py-2.5 px-5 text-gray-500">
            ...
            <div className="p-3 flex-1 flex flex-col justify-between">
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className="bg-blue-50/45 p-3 rounded-lg border border-blue-100/60">
                  <h3 className="text-2xl font-black text-blue-600">{totalRepairs}</h3>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mt-1">Total Repairs</p>
                </div>
                <div className="bg-emerald-50/45 p-3 rounded-lg border border-emerald-100/65">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mt-2">Lifetime Value</p>
                </div>
              </div>
              ...
              {lastRepairDate && (
                <div className="border-t border-gray-100 pt-3 flex justify-between items-center text-[12px] text-gray-600 bg-gray-50/30 p-2.5 rounded-lg border border-gray-100/40 mt-4">
                  <span className="font-semibold text-gray-400 text-[11px] uppercase tracking-wide">Last Repair Date</span>
                  <span className="font-bold text-gray-800">
                    {lastRepairDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </span>
                </div>
              )}
            </div>
