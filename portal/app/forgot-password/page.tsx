...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert('If the email is registered, a password reset link has been generated.');
  };
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '100px auto', padding: '30px', backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        <div>
          <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px', color: '#94a3b8' }}>Email Address</label>
          <input
            type="email"
            value={email}
      ...
      <div style={{ marginTop: '15px', textAlign: 'center' }}>
        <a href="/login" style={{ color: '#3b82f6', fontSize: '14px', textDecoration: 'none' }}>Back to Login</a>
      </div>
    </div>
  );
}
